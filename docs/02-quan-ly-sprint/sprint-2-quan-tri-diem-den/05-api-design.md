...
### 3.5. Lấy danh sách Vùng miền — [UPDATED 17/05/2026]

**Triển khai web:** Server Action `getRegionsAction()` (admin), `DestinationService.getRegions()` — **không** có `GET /api/v1/regions`.

- **Response**: Mảng 3 vùng miền cố định (Bắc, Trung, Nam) từ DB seed.
- Dùng cho dropdown Admin và lọc Client.

### ~~3.6 – 3.8. CRUD Vùng miền~~ — Không triển khai

Danh mục vùng miền **cố định**; không có `POST` / `PUT` / `DELETE` cho `regions`. Nếu sau này mở rộng thị trường, cập nhật qua migration + seed, không qua UI admin.
