...
| AC-07 | Nút Đặt chuyển hướng tới `/dat-tour/[bookingId]/thanh-toan` (chỗ tạm) | Sprint 6 sẽ làm trang thanh toán thật |
...
| AC-04 | `00-san-pham/04-lo-trinh-phat-hanh.md` cập nhật mốc thời gian (Sprint 4 Đổi nền xong, các Sprint 5-8 lùi 2 tuần) | Cập nhật lộ trình |
| AC-05 | Sprint 6 (Đặt chỗ) ở `05-quan-ly-sprint/sprint-6-dat-cho-thanh-toan/` rà soát lại + cập nhật theo cấu trúc dữ liệu mới | Loại các tham chiếu cấu trúc dữ liệu cũ — checklist bên dưới |
| AC-06 | Sprint retro note: "what went well / not / next time" | Mini doc 1 trang |

### Việc cần làm cho Sprint 6 (chi tiết AC-05)

Mỗi file dưới đây cần kiểm tra (grep) + đánh dấu chỗ cần cập nhật:

| File | Pattern grep | Action |
| --- | --- | --- |
| `sprint-6-dat-cho-thanh-toan/01-yeu-cau-nghiep-vu.md` | `bookingType / HotelBooking / unitPrice / participants` | Replace bằng `priceBreakdown / adults / children / infants` |
| `sprint-6-dat-cho-thanh-toan/02-thiet-ke-co-ban.md` | Same + `priceFrom / durationText / tourType` | Replace bằng `priceAdult / durationDays / tourType enum` |
| `sprint-6-dat-cho-thanh-toan/03-thiet-ke-chi-tiet.md` | Same + `HotelBooking model / hotelBooking relation` | Drop hoàn toàn |
| `sprint-6-dat-cho-thanh-toan/04-api-design.md` | `POST /bookings` request body | Replace `{ participants }` bằng `{ adults, children, infants, optionId, isSingleSupplement }` |
| `sprint-6-dat-cho-thanh-toan/05-admin-specs.md` | `BookingType column` | Drop column |
| `sprint-6-dat-cho-thanh-toan/06-client-app-specs.md` | Booking widget UI | Reference `04-wireframes.md` mới |

```bash
cd travel-doc/docs/05-quan-ly-sprint/sprint-6-dat-cho-thanh-toan
...
05-quan-ly-sprint/sprint-6-dat-cho-thanh-toan/        ← audit schema refs
...
- [ ] Roadmap update
- [ ] Sprint 6 spec đã rà soát + đánh dấu chỗ cần cập nhật (chưa cần viết lại)
- [ ] Changelog entry
