...
**Điều kiện cần trước Day 1** (Path A — chưa có khách thật):
- [ ] Postgres local đã chạy (psql kết nối được tới localhost)
- [ ] Dump backup Supabase production một bản về `backups/` (đề phòng)
- [ ] Tạo database `vivu_test` trên Postgres local (cho test infra Sprint 4 sau này)
- [ ] Wireframe Booking widget đã đọc (xem `sprint-4-schema-pivot/04-wireframes.md`)
- [ ] Đọc `02-runbook.md` để hiểu 13 nhóm thay đổi prisma/schema.prisma

> Khi nào cần Supabase Pro + PITR + staging? → Sau khi Sprint 6 ra mắt nội bộ có khách thật. Lúc đó dùng `sprint-4-schema-pivot/02-runbook-luc-da-co-khach.md` làm template.
